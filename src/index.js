...
const setBeerBarAbv = () => {
    ...
    barChart.selectAll()
        ...
        .attr('y', (beer) => {
            ...
            const avgAbv = parseFloat(abvMax + abvMin) / 2;
            console.log(typeof avgAbv);
            return yScale(avgAbv);
        })
        .attr('height', (beer) => {
            if(beer.abv) {
                return height - yScale(beer.abv);
            } else {
                const { abvMin, abvMax } = beer.style;
                const abvAvg = parseFloat(abvMax + abvMin) / 2;
                return height - yScale(abvAvg);
            }
        })
        .attr('width', xScale.bandwidth())
}